...
<div class="p-4 md:p-6 pb-20 md:pb-6">
    ...
    <?php elseif ($action === 'create'): ?>
        ...
        // Add this section after the existing payment form in payments.php
<?php if ($action === 'create' && $saleInfo): ?>
<!-- Online Payment Options -->
<div class="bg-white rounded-lg shadow p-6 mt-6">
    <h3 class="text-lg font-bold mb-4">Online Payment Options</h3>
    
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <!-- M-Pesa -->
        <button onclick="initiatePayment('mpesa', <?php echo $saleInfo['balance']; ?>, <?php echo $saleId; ?>)" 
                class="p-6 border-2 border-gray-200 rounded-lg hover:border-green-500 hover:bg-green-50 transition">
            <img src="/assets/images/mpesa-logo.png" alt="M-Pesa" class="h-12 mx-auto mb-3">
            <p class="font-semibold">Pay with M-Pesa</p>
            <p class="text-xs text-gray-600 mt-1">Instant payment via Lipa Na M-Pesa</p>
        </button>
        ...
        <button onclick="initiatePayment('card', <?php echo $saleInfo['balance']; ?>, <?php echo $saleId; ?>)" 
            ...
            <p class="font-semibold">Pay with Card</p>
            <p class="text-xs text-gray-600 mt-1">Visa, Mastercard, American Express</p>
        </button>
    </div>
</div>
...
function initiatePayment(gateway, amount, saleId) {
    if (gateway === 'mpesa') {
        ...
        // Redirect to Stripe checkout or show card form
        window.location.href = `/payments/card-checkout.php?sale_id=${saleId}&amount=${amount}`;
    }
}
